...
const profiles: ProjectAnalyticsProfile[] = [
  {
    id: 'vows-growth-v1',
    version: 1,
    projectId: 'vows',
    ...
    metrics: [
      ...
      { key: 'paymentOrders', label: '创建支付订单', funnelStage: '创建支付订单', source: 'payment_orders.created_at', unit: 'count' },
      { key: 'paidOrders', label: '支付成功订单', funnelStage: '支付成功', source: 'payment_orders.paid_at', unit: 'count' },
      { key: 'paidRevenueCny', label: '支付金额', funnelStage: '支付成功', source: 'payment_orders.amount_total', unit: 'CNY' },
      { key: 'weddingsCreated', label: '创建婚礼', funnelStage: '创建婚礼', source: 'weddings.created_at', unit: 'count' },
      { key: 'publishedWeddingEvents', label: '已发布邀请', funnelStage: '邀请就绪/发布', source: 'wedding_events.published', unit: 'count' },
  ...
  },
  {
    ...
    metrics: [
      { key: 'imageCreations', label: '图片工作流', funnelStage: '创建工作流', source: 'image_creations.created_at', unit: 'count' },
      { key: 'generationJobs', label: '生成任务', funnelStage: '生成任务', source: 'generation_jobs.created_at', unit: 'count' },
      { key: 'completedJobs', label: '完成任务', funnelStage: '生成任务', source: 'generation_jobs.status', unit: 'count' },
      { key: 'failedJobs', label: '失败任务', funnelStage: '生成任务', source: 'generation_jobs.status', unit: 'count' },
      { key: 'imageCandidates', label: '候选素材', funnelStage: '候选素材', source: 'image_candidates.created_at', unit: 'count' },
      { key: 'adoptedReviews', label: '采纳评审', funnelStage: '候选采纳', source: 'image_candidate_reviews.decision', unit: 'count' },
      { key: 'readyImageDeliverables', label: '图片交付物', funnelStage: '最终交付', source: 'image_creation_deliverables.status', unit: 'count' },
      { key: 'finalVideoDeliverables', label: '视频交付物', funnelStage: '最终交付', source: 'video_deliverables.status', unit: 'count' }
    ],
    requiredConnectors: ['postgres', 'repo', 'project-agent'],
...
export function listProjectAnalyticsProfiles(): ProjectAnalyticsProfile[] {
  return profiles.map((profile) => ({ ...profile, metrics: profile.metrics.map((metric) => ({ ...metric })) }))
}
